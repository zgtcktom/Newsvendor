...
function popup(form, hiddenFields, displayNames) {
  ...
  for (let field of Object.keys(form)) {
    ...
    input.setAttribute(
      "type",
      typeof form[field] == "number" ? "number" : "text"
    );
...
function isPreTrained() {
  return (
    document.getElementById("use-policy")["use-policy"].value == "pre-trained"
  );
}
...
function openSettings() {
  function submitSettings(data) {
    ...
    let [chart, updateChart, destroyChart] = (function () {
      let getLabels = () => {
        return new Array(order_history.length)
          .fill(0)
      ...
      return new Promise((resolve, reject) => {
        ...
        let step = (timestamp) => {
          ...
          progress +=
            previousTimestamp == -1
              ? 0
              : (timestamp - previousTimestamp) / duration;
        ...
        if (isManual()) {
          ...
          confirm2.onclick = () => {
            ...
            if (record.overstock) {
              ...
              understockCosts.innerHTML += ` (+${record.understock.toFixed(
                1
              )})`;
            }
            ...
            let row = manualMapping.find(
              (row) => row[0] == manualInput.amount()
            );
            if (row == undefined) {
...
class GridSystem {
  ...
  update(playground, autoUpdate = false) {
    ...
    let [centerX, centerY] = [
      playground.offsetWidth / 2,
      playground.offsetHeight / 2,
    ];
...
console.log(
  mergesort([1, "asd", null, {}, function () {}, 99, "ok", -1, true])
);
console.log(true);
...
console.log(
  "shortestPath",
  gridSystem.shortestPath(startPosition, endPosition)
);
...
let animate = (
  ...
  let step = (timestamp) => {
    if (prevTimestamp != -1)
      progress = Math.min(1, progress + (timestamp - prevTimestamp) / duration);
...
function getScreenPosition(gridPosition) {
  let [centerX, centerY] = [
    playground.offsetWidth * 0.5,
    playground.offsetHeight * 0.5,
  ];
  let screenPosition = gridSystem.getPosition(add(gridPosition, [0.5, 0.5]));
  screenPosition = add(screenPosition, [centerX, centerY]);
  screenPosition = sub(screenPosition, [
    truckNorth.offsetWidth * 0.5,
    truckNorth.offsetHeight * 0.5,
  ]);
...
function animatePath(path, duration = 3000) {
  ...
  let totalDistance = path
    .slice(1)
    .reduce((accum, node, index) => accum + distance(path[index], node), 0);
  ...
  for (let i = 0; i < path.length - 1; i++) {
    chain = chain.then(() => {
      ...
      state.handler = truckAnimate(path[i], path[i + 1], d, prevTruck, () => {
        ...
        if (
          !new Set([truckEast, truckWest, truckNorth, truckSouth]).has(
            prevTruck
          )
        ) {
          prevTruck.remove();
        ...
        if (prevTruck != null && prevTruck != truck)
          prevTruck.style.display = "none";
...
let truckPath = gridSystem
  .shortestPath(startPosition, endPosition)
  .map((e) => e[0]);
