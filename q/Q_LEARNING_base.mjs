...
export class RL_brain_base {
	...
	build_q_table(n_states, actions) {
		let table = DataFrame(
			...
			actions,
			range(-30 | 0, (this.N_STATES - 30) | 0) //# actions's name
		);
	...
	simulation(S, R, a, episode, LOST) {
		...
		if (
			episode <= 182 ||
			(365 <= episode && episode <= 182 * 3 + 1) ||
			365 * 2 <= episode <= 182 * 5 + 1
		) {
		...
		if (episode >= this.LEAD_TIME - 1) {
			if (S + getitem(a, [-this.LEAD_TIME]) - lost_ >= demand) {
				// on hand
		...
		IP_ = Math.min(
			Math.max(S_ - lost + sum(a.slice(-(this.LEAD_TIME - 1))), -(30 | 0)),
			(this.N_STATES - 30 - 1) | 0
		);
		R = Ro + Rs;
		return [S_, IP_, IL_, R, lost, demand, Ro, Rs];
	}
	...
	*rliter(episodeMode) {
		...
		// the outer loop
		// let ret = [q_table_1, q_table_2, R_episode, simulation_time, Inv_state, a, Onhand_state];
		// yield ret;
		for (let i = 0; i < this.SIMULATION_TIME; i++) {
			...
			for (let episode = 0; episode < this.MAX_EPISODES; episode++) {
				...
				let S_, IP_, a_, q_predict, q_target, demand, Ro, Rs;
				[S_, IP_, IL_, R, lost, demand, Ro, Rs] = this.simulation(
					S[episode],
					R,
					a,
					episode,
				...
				if (
					episode <= 182 ||
					(365 <= episode && episode <= 182 * 3 + 1) ||
					(365 * 2 <= episode && episode <= 182 * 5 + 1)
				) {
					DEMAND1.push(demand);
					r_estimate = this.getSafetyStock(DEMAND1);
					[a_, Is_greedy] = ActionNumber.choose_action1(
						IP_,
						q_table_1,
						EPSILON,
						r_estimate,
					...
					if (episode <= this.LEAD_TIME - 1) {
						...
						episode <= 182 - this.LEAD_TIME ||
						(365 <= episode && episode <= 182 * 3 + 1 - this.LEAD_TIME) ||
						(365 * 2 <= episode && episode <= 182 * 5 + 1 - this.LEAD_TIME)
					) {
						q_predict = getitem(q_table_1, [
							getitem(IP, [-this.LEAD_TIME]),
							getitem(a, [-this.LEAD_TIME]),
						]);
						R_count = R_count + R;
						q_target =
							R +
							GAMMA * Math.max(...getitem(q_table_1, [getitem(IP, [-(this.LEAD_TIME - 1)])]));
						setitem(
							q_table_1,
							[getitem(IP, [-this.LEAD_TIME]), getitem(a, [-this.LEAD_TIME])],
							getitem(q_table_1, [
								getitem(IP, [-this.LEAD_TIME]),
								getitem(a, [-this.LEAD_TIME]),
							]) +
								ALPHA * (q_target - q_predict)
						); // update
						Q_values = getitem(q_table_1, [
							getitem(IP, [-this.LEAD_TIME]),
							getitem(a, [-this.LEAD_TIME]),
						]);
					...
					[a_, Is_greedy] = ActionNumber.choose_action2(
						IP_,
						q_table_2,
						EPSILON,
						r_estimate,
					...
					if (episode <= this.LEAD_TIME - 1) {
						...
						episode <= 365 - this.LEAD_TIME ||
						episode <= 730 - this.LEAD_TIME ||
						episode <= 1095 - this.LEAD_TIME
					) {
						q_predict = getitem(q_table_2, [
							getitem(IP, [-this.LEAD_TIME]),
							getitem(a, [-this.LEAD_TIME]),
						]);
						R_count = R_count + R;
						q_target =
							R +
							GAMMA * Math.max(...getitem(q_table_2, [getitem(IP, [-(this.LEAD_TIME - 1)])]));
						setitem(
							q_table_2,
							[getitem(IP, [-this.LEAD_TIME]), getitem(a, [-this.LEAD_TIME])],
							getitem(q_table_2, [
				...
				backlogCosts.push(Rs);
				inventoryCosts.push(Ro);
				if (episodeMode) {
					let ret = [
						q_table_1,
						q_table_2,
	...
	rl(episodeMode = false) {
		let q_table_1, q_table_2, R_episode, simulation_time, Inv_state, a, Onhand_state, DEMAND;

		for (let ret of this.rliter(episodeMode)) {
			[q_table_1, q_table_2, R_episode, simulation_time, Inv_state, a, Onhand_state, DEMAND] = ret;
		}
		return [q_table_1, q_table_2, R_episode, simulation_time, Inv_state, a, Onhand_state, DEMAND];
	}
}
