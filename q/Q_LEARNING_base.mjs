...
export class RL_brain_base {
	...
	*rliter() {
		...
		for (let i = 0; i < this.SIMULATION_TIME; i++) {
			...
			let ret = [q_table_1, q_table_2, R_episode, simulation_time, IP, a, S];
			yield ret;
		}
	}

	rl() {
		let q_table_1, q_table_2, R_episode, simulation_time, Inv_state, a, Onhand_state;

		for (let ret of this.rliter()) {
			[q_table_1, q_table_2, R_episode, simulation_time, Inv_state, a, Onhand_state] = ret;
		}
